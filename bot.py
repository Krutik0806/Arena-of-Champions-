...
async def safe_edit_message(query, text: str, **kwargs) -> bool:
    ...
    try:
        ...
        if "message is not modified" in error_msg or "message_not_modified" in error_msg:
            # Same content, ignore - this is not an error
            ...
            # Message was deleted or too old, send new message
...
class InputValidator:
    ...
    def sanitize_text(cls, text: str, max_length: int = None, html_escape: bool = True) -> str:
        ...
        # HTML escape for safety (optional)
        if html_escape:
...
# Old duplicate functions removed - use InputValidator class methods instead
...
class SmartCacheManager:
    ...
    def should_invalidate_cache(cache_entry: dict, current_time: float, cache_duration: int, 
        ...
        # Check if cache was globally invalidated
    ...
    def invalidate_related_caches(bot_instance, operation_type: str, user_id: int = None):
        ...
        if operation_type == "profile_update" and user_id:
            # Invalidate specific user's profile cache
            ...
            # Invalidate all profile caches (achievements affect profiles)
            if hasattr(bot_instance, 'profile_cache'):
...
# Configure basic logging (but keep our StructuredLogger as main logger)
logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    level=logging.INFO
)
# Note: We keep logger = StructuredLogger("ChatBot") from above
...
async def safe_send(call, *args, **kwargs):
    ...
    for i in range(retries):
        try:
            ...
            logger.warning(f"Rate limited, retrying after {e.retry_after}s (attempt {i+1}/{retries})")
            await asyncio.sleep(e.retry_after + 1)
        except (TimedOut, NetworkError) as e:
            logger.warning(f"Network error: {type(e).__name__} (attempt {i+1}/{retries})")
            await asyncio.sleep(1 + i)
        except Exception as e:
            logger.error(f"Error in safe_send: {type(e).__name__}: {e} (attempt {i+1}/{retries})")
...
def db_query(read_only=False, return_conn=False, fallback=None):
    ...
    Usage:
        ...
        def complex_operation(self, cursor, conn, data):
            # Access to both cursor and connection for complex operations
    ...
    def decorator(func):
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                # Pass cursor (and optionally conn) as first argument(s) after self
                if return_conn:
...
def check_banned(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        try:
            ...
            # Skip checks for start command to allow showing join buttons
            if func.__name__ == 'start_command':
                return await func(update, context, *args, **kwargs)
            
            # CRITICAL: Only perform checks for direct commands (starting with /)
            # Don't check for mentions, tags, callbacks, or non-command messages
            is_command = (update.message and update.message.text and 
            ...
            # Check if user is registered (except for admin commands and passive handlers)
            ...
            # Handlers that don't require registration (passive/auction system)
            passive_handlers = ['handle_manual_auction_input', 'handle_auction_registration',
                               'handle_callback_query', 'auction_callback_router', 
                               'handle_host_panel_callbacks', 'handle_admin_auction_approval',
                               'handle_auction_sale_callbacks', 'handle_captain_approval_callbacks',
                               'handle_player_approval_callbacks', 'track_message',
                               'button_click', 'guess_callback', 'admin_panel_callback']
            
            # Check registration for commands (already verified is_command above)
            if func.__name__ not in admin_commands and func.__name__ not in passive_handlers:
                # Single canonical registration check - avoids duplicate lookups
                player = bot_instance.find_player_by_identifier(str(user_id))
                if not player:
                    await update.message.reply_text(
                        "⚠️ <b>START REQUIRED!</b>\n\n"
                        "You must start the bot in private chat first before using commands.\n\n"
                        "👉 Click here: @" + context.bot.username + "\n"
                        "Then send /start to register!",
                        parse_mode='HTML'
                    )
                    return
            
            # User is not banned and registered (or is admin), proceed with command            # User is not banned and registered (or is admin), proceed with command
            return await func(update, context, *args, **kwargs)
        except telegram_error.BadRequest as e:
            # Handle Telegram API errors (message not modified, etc.) - don't show error to user
            error_msg = str(e).lower()
            if any(ignore in error_msg for ignore in ['message is not modified', 'message_not_modified', 
                                                       'message to edit not found', 'message can\'t be edited']):
                logger.debug(f"Telegram API: {e}")
                return
            else:
                logger.error(f"Telegram BadRequest in check_banned: {e}")
                raise
        except Exception as e:
            logger.error(f"Error in check_banned decorator: {e}")
            # Only show generic error for unexpected exceptions
            try:
            ...
            async with aiohttp.ClientSession() as session:
                while self.running:
                    try:
                        async with session.get(self.url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                            if response.status != 200:
                                ...
                                if consecutive_failures >= max_failures:
                                    logger.critical("Health check failed multiple times. Signaling graceful shutdown.")
                                    # Signal graceful shutdown instead of hard exit
                                    self.running = False
                                    return
                            else:
                        ...
                        if consecutive_failures >= max_failures:
                            logger.critical("Watchdog encountered critical errors. Signaling graceful shutdown.")
                            self.running = False
                            return
...
class ArenaOfChampionsBot:
    def __init__(self):
        # Initialize fully thread-safe roast cache
        ...
        # Initialize thread-safe leaderboard cache with invalidation support
        ...
        # Initialize thread-safe profile cache with invalidation support
        self.profile_cache = {
    ...
    def _initialize_database_tables(self) -> None:
        ...
        try:
            ...
            if conn:
                ...
                # Create banned_users table for user management
                cursor.execute("""
                ...
                # Use transaction to atomically drop and recreate views
                ...
                # Create indexes for auction tables
    ...
    def _setup_migrations(self):
        ...
        try:
            ...
            # Add shards columns to players table
            migration_system.add_migration(
                version=2,
    ...
    async def execute_with_retry(self, operation, *args, retries=2, timeout=5):
        ...
        for attempt in range(retries + 1):
            try:
                # Validate operation is callable before executing
            ...
            if self.db_pool:
                ...
                try:
                    ...
                    if pool_size > 22:  # Warn if pool usage is high (>87% of 25)
                        ...
                        # Schedule admin log for high pool usage (async-safe)
                        try:
                            ...
                            # Schedule using deferred task scheduler
    ...
    def init_database(self):
        ...
        try:
            ...
            # Chase games table - for chase game tracking
            cursor.execute("""
            ...
            # Guess games table - for guess game tracking  
            cursor.execute("""
    ...
    def is_admin(self, user_id: int) -> bool:
        ...
        # Level 2: Environment-defined admins (permanent)
        if user_id in self.admin_ids:
            logger.debug(f"User {user_id} verified as ENV_ADMIN")
            return True
        
        # Level 3: Database admins (dynamic, fallback to env if DB unavailable)
        try:
            with self.get_db_connection_ctx() as conn:
                if not conn:
            ...
            # Fallback: deny access if database check fails for security
            return False
                ...
                for admin_id in self.admin_ids:
                    try:
                        ...
                        if not cursor.fetchone():
                            # Add environment admin to database
                            cursor.execute("""
    ...
    def get_chase_leaderboard(self, limit: int = 10) -> List[Dict]:
        ...
        """
        # Check cache first (respecting manual invalidation)
        current_time = time.time()
        cache_valid = (
    ...
    def refresh_roast_cache(self) -> bool:
        ...
        with self.get_db_connection_ctx() as conn:
            ...
            try:
                ...
                # Clear and repopulate usage counts atomically
    ...
    def get_cached_roast_line(self) -> str:
        ...
        # Check if cache needs refresh (thread-safe)
        ...
        # Get data with atomic operation to ensure consistency
        ...
        # Thread-safe operations to find minimum usage
        min_usage = usage_counts.get_min_value()
        
        # Get all lines with minimum usage (thread-safe)
        available_lines = usage_counts.get_keys_with_min_value()
        
        # Filter to only include lines that are actually in our cache
    ...
    def get_cached_profile_data(self, user_id: int) -> dict:
        ...
        if (user_id in self.profile_cache['data'] and 
            ...
            # Use SmartCacheManager to determine if cache should be invalidated
            cache_entry = {
            ...
            if not should_invalidate:
                # Verify data hasn't changed by comparing hash
        ...
        try:
            ...
            # Cache the data with hash for change detection
            profile_data = {
                'player': player,
            ...
            # Generate hash for future comparison
    ...
    def update_roast_usage_async(self, roast_line: str):
        ...
        async def update_usage_task():
            try:
                ...
                # Update local cache with thread-safe increment
    ...
    def format_achievements_message(self, player: Dict, achievements: List[Tuple[str, int]]) -> str:
        ...
        for index, (achievement, count) in enumerate(achievements, 1):
            # Get proper case name for display
    ...
    def generate_nightmare_hint(self, number: int) -> dict:
        ...
        # Create cryptic mathematical hints for 1-20000 range
        hint_variants = []
        ...
        # More cryptic digit-specific hints
        if len(digits) > 1:
            # LESS revealing digit hints for higher difficulty
        ...
        # Enhanced divisibility hints (mathematical challenge)
        divisibility_hints = []
        if number % 3 == 0:
        ...
        # Product of digits (if not too revealing)
        digit_product = 1
        for d in digits:
            digit_product *= d
        ...
        # Pick a random hint - ensuring we have good variety
        ...
        try:
            ...
            if game_over:
                ...
                if won:
                    ...
                    # Log nightmare victory (async-safe)
                    try:
    ...
    def register_captain_request(self, auction_id: int, user_id: int, name: str, team_name: str) -> bool:
        ...
        # Check if user is already registered as player
    ...
    def register_player_request(self, auction_id: int, user_id: int, name: str, username: str = None) -> bool:
        ...
        # Check if user is already registered as captain
    ...
    def start_auction_bidding(self, auction_id: int) -> bool:
        ...
        # Use the shuffled player_queue instead of approved_players keys
        ...
        # Use player_queue which is already shuffled
    ...
    def _record_auction_result(self, player, status: str, amount: int, captain=None):
        """
        Helper to create consistent auction result records.
        Prevents format drift and ensures captain_id is always included.
        
        Args:
            player: Player object
            status: 'SOLD' or 'UNSOLD'
            amount: Sale amount (0 for unsold)
            captain: Captain object (None for unsold)
        
        Returns:
            Dict with keys: player, team, amount, captain, captain_id
        """
        return {
            'player': player,
            'team': captain.team_name if captain else 'UNSOLD',
            'amount': amount,
            'captain': captain.name if captain else None,
            'captain_id': captain.user_id if captain else None
        }
    ...
    def sell_current_player(self, auction_id: int) -> bool:
        ...
        # LOCK PROTECTION: Prevent concurrent modifications to queue/sold_players
        ...
        try:
            # Capture current player reference BEFORE any state changes
            sold_player = auction.current_player
            
            if not auction.current_bids:
                # No bids, player UNSOLD
                # Store as UNSOLD (critical for tracking)
            ...
            # Find highest bidder from current_bids (source of truth)
            ...
            # Update player using captured reference
            ...
            # Move to sold players using captured reference
            if not hasattr(auction, 'sold_players'):
                auction.sold_players = {}
            auction.sold_players[sold_player.user_id] = self._record_auction_result(
    ...
    def skip_current_player(self, auction_id: int) -> bool:
        ...
        try:
            ...
            # Store in unsold_players (separate from sold_players for tracking)
            if not hasattr(auction, 'unsold_players'):
                auction.unsold_players = {}
            auction.unsold_players[current.user_id] = {
                'player': current,
                'team': 'UNSOLD',
                'amount': 0,
                'captain': None
    ...
    def assign_player_manually(self, auction_id: int, player_id: int, captain_id: int, amount: int) -> bool:
        ...
        # CRITICAL: Validate purse BEFORE any modifications
        ...
        # LOCK PROTECTION: Prevent concurrent modifications
        lock_acquired = auction._bid_lock.acquire(blocking=True, timeout=5)
        if not lock_acquired:
        ...
        try:
            ...
            # Move to sold players (consistent format with sell_current_player)
            if not hasattr(auction, 'sold_players'):
                auction.sold_players = {}
            auction.sold_players[player_id] = self._record_auction_result(
    ...
    def rebid_player(self, auction_id: int, player_username: str) -> tuple:
        ...
        for pid, player_data in auction.sold_players.items():
            ...
            if p_username == player_username.lower().replace('@', '') or p.name.lower() == player_username.lower().replace('@', ''):
                player = p
                player_id = pid
                # Remove from sold and refund captain
                captain_name = player_data['captain']
                # Find captain object
                captain = auction.approved_captains.get(player_data.get('captain_id')) or next(
                    (c for c in auction.approved_captains.values() if c.name == captain_name), None
                )
                if captain:
                    captain.purse += player_data['amount']
                    captain.spent -= player_data['amount']
                    # captain.players stores names, not objects
                    if player.name in captain.players:
                        captain.players.remove(player.name)
    ...
    def assign_unsold_player(self, auction_id: int, player_username: str, team_name: str, amount: int) -> tuple:
        ...
        player = None
        player_id = None
        for pid, player_data in auction.unsold_players.items():
            # Handle both dict format and legacy direct object format
            p = player_data['player'] if isinstance(player_data, dict) else player_data
            p_username = getattr(p, 'username', '').lower() if hasattr(p, 'username') else ''
            if p_username == player_username.lower().replace('@', '') or p.name.lower() == player_username.lower().replace('@', ''):
        ...
        # Store in consistent format with other sold_players
        auction.sold_players[player_id] = self._record_auction_result(
            player, 'SOLD', amount, captain
        )
...
async def nightmare_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Check if this action requires an active game
        if data.startswith('nightmare_quit_') or data == 'nightmare_cancel':
            # These require active game - validate user
            active_game = bot_instance.get_nightmare_game(user_id)
            if not active_game:
                await query.answer("⚠️ You don't have an active nightmare game!", show_alert=True)
                return
        
        # For game completion buttons (retry), validate the message belongs to this user
        if data == 'nightmare_retry':
            # Check if user can start a new game (message might be from their previous game)
            # Allow retry for anyone, but they start their own game
        ...
        # Ignore message edit errors silently
    except Exception as e:
...
async def schedule_daily_rewards():
    ...
    # Use /ddrlb command to manually distribute rewards
...
async def commands_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Define command categories based on user role
    if is_super_admin:
    ...
    for i, category in enumerate(categories):
        ...
        # Add row when we have 2 buttons or at the end
...
async def chase_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Safety check: ensure query.message exists
    if not query.message:
        logger.warning("chase_callback called with no query.message")
        return
    ...
    try:
        # Generate new UI with updated information
        text = _format_chase_card(state, last)
        
        # Force update by ensuring message content changes
        await query.edit_message_text(text, parse_mode="HTML", reply_markup=_chase_keyboard())
        ...
        if "message is not modified" in error_msg:
            ...
            # Try to force update by regenerating with fresh timestamp
            try:
...
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Start message with enhanced Arena theme
...
async def admin_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Check for admins only in database (not in env)
        ...
        # Check for env admins not synced to DB
...
async def cleancache_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Clear various caches using SmartCacheManager
        cache_cleared = 0
        
        # Profile cache with new data versioning
...
async def thread_safety_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Check profile cache structures
        if hasattr(bot_instance, 'profile_cache'):
            cache = bot_instance.profile_cache
        ...
        # Check roast cache usage counts
...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Build profile message with enhanced Arena theme
...
⚡ <b>Bot Status:</b> ✅ Running"""
        ...
        # List all auctions with management options
        ...
        # Show active auctions that can be force ended
        ...
        # Show pending auction proposals
        ...
        # Show comprehensive auction system statistics
        total_proposals = len(bot_instance.auction_proposals)
        total_approved = len(bot_instance.approved_auctions)
...
async def reset_all_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Perform the selective reset using direct database operations
    conn = bot_instance.get_db_connection()
    if not conn:
    ...
    try:
        ...
        # Reset shard balances to 0
        cursor.execute("UPDATE players SET shards = 0")
        
        # Reset daily streaks (only if columns exist)
        try:
            # Check if daily_streak column exists first
            cursor.execute("""
        ...
        try:
            # Check if last_daily_claim column exists first  
            cursor.execute("""
                SELECT column_name FROM information_schema.columns 
        ...
        # Clear transaction history if table exists
        cursor.execute("""
            SELECT EXISTS (
        ...
        # Clear daily shard bonuses if table exists
        cursor.execute("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_schema = 'public' 
...
# Duplicate functions removed - using the more comprehensive implementations above
...
async def guess_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Check if this is a hint button - requires active game
        if data == 'guess_hint':
        ...
        # Safety check: ensure query.message exists for operations that need it
        if not query.message:
            logger.warning("guess_callback called with no query.message")
            return
...
async def handle_manual_auction_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # OPTIMIZATION: Quick exit if no active auctions at all
        if not bot_instance.approved_auctions:
            return
        
        # OPTIMIZATION: Check if there's any active auction first
        ...
        # Find active auctions where this user is a captain or admin
        active_auction = None
        user_captain = None
        ...
        for auction_id, auction in bot_instance.approved_auctions.items():
            if auction.status == "active" and auction.current_player:
                ...
                for captain in auction.approved_captains.values():
                    # Get captain ID from user_id attribute  
        ...
        # Log only when auction handler actually processes something
        logger.debug(f"Auction handler processing - User: {user.id}, Text: '{text}', Auction: {active_auction.id}")
        
        # Double-check auction is still active (prevent race condition)
        ...
        # Debug: Check all auctions and their states
        ...
        for aid, auction in bot_instance.approved_auctions.items():
            ...
            # Check if this user is a captain in any auction
        ...
        if user_captain:
            ...
            if bid_amount:
                # Check if current player still exists (might have been sold/moved)
                ...
                try:
                    ...
                    # Store bid in current_bids for sell_current_player method
...
async def auction_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Get captain info if user is captain
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Handle channel membership check (doesn't require admin)
    if data == "check_membership":
        user_id = query.from_user.id
        is_member, not_joined = await check_channel_membership(user_id, context.bot)
        
        if is_member:
            # User has joined all channels, trigger start command
            await query.edit_message_text(
                "✅ <b>MEMBERSHIP VERIFIED!</b>\n\n"
                "Welcome to Arena of Champions! 🎉\n\n"
                "Please send /start again to complete registration.",
                parse_mode='HTML'
            )
        else:
    ...
    try:
        ...
        # Handle Telegram API errors silently for common issues
        error_msg = str(e).lower()
        if any(ignore in error_msg for ignore in ['message is not modified', 'message_not_modified',
                                                   'message to edit not found', 'message can\'t be edited']):
            logger.debug(f"Telegram API in callback: {e}")
        else:
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if data == "start_guess":
            ...
            # Show guess game stats (this should be handled by admin panel, but just in case)
            await guess_stats_command(update, None)
            
        elif data == "action_chasestats":
            # Show chase game stats (this should be handled by admin panel, but just in case)
            await chase_stats_command(update, None)
            
        elif data == "daily_challenge":
            # Start daily challenge (redirect to guess game with daily flag)
            await query.edit_message_text(
        ...
        try:
            # Safely access message text with fallback
            original_text = getattr(query.message, 'text', 'Action') if query.message else 'Action'
            await query.edit_message_text(
                text=original_text + "\n\n❌ Error processing action",
                parse_mode='HTML'
            )
        except Exception:
            # If even the error handler fails, just answer the callback
            try:
...
💰 <b>Welcome Bonus:</b> {welcome_bonus} Shards! 💠
🎁 Your journey begins NOW!
🏆 Prove yourself in epic challenges
...
# ====================================
# NEW AUCTION SYSTEM COMMAND HANDLERS
# ====================================
...
async def register_auction_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Create new proposal and start registration process
        ...
        # Store proposal ID for this user
...
async def handle_auction_registration(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if state.step == "name":
            ...
            # Process both purse and base price together (in crores)
            try:
...
async def hostpanel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if auction.status == "setup":
            ...
            # Stage 2: Captain registration active, can start player registration
            keyboard.extend([
            ...
            # Stage 3: Both captain and player registration active
            keyboard.extend([
                [InlineKeyboardButton("✅ Approve Captains", callback_data=f"host_approve_captains_{auction_id}")],
...
async def registercaptain_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Allow captain registration during captain_reg, player_reg, or ready status
        ...
        if success:
            # Notify host with individual approval buttons
            keyboard = [
                [
...
async def registerplayer_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Allow player registration during captain_reg, player_reg, or ready status
        if auction.status not in ["captain_reg", "player_reg", "ready"]:
        ...
        if success:
            # Instantly notify host with approve/reject buttons
            keyboard = [
                [
    ...
    try:
        ...
        if data.startswith("host_start_captain_"):
            ...
            if success:
                ...
                # Single short auction message only
            ...
            # Get username or name for rebid
        ...
        if data.startswith("confirm_sale_"):
            ...
            try:
                ...
                if auction.highest_bidder and auction.current_player:
                    # Find the captain using user_id
...
async def myteam_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Find auction where user is captain
        user_auction = None
        captain = None
        ...
        for i, player_name in enumerate(captain.players, 1):
            # captain.players contains player names (strings)
...
async def transfercap_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Check if user is a captain in this auction
        if user.id not in auction.approved_captains:
            await update.message.reply_text("❌ You are not a captain in this auction!")
            return
        ...
        # Get new captain identifier (user ID or username)
        new_captain_identifier = context.args[1]
        
        # Try to find the new captain
        new_captain_id = None
        ...
        # Check if new captain is already a captain
        ...
        # Check if new captain is a player
        ...
        # Create new approved captain with proper class
        ...
        # Copy all the team data from old captain
        ...
        # Update sold_players records if they exist
...
async def auction_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if success:
            ...
            # Send to group chat if set (only if different from current chat)
...
async def sell_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Trigger the same confirmation as ".." system
        if auction.highest_bidder:
            ...
            # CRITICAL: Store as UNSOLD (consistent format)
            if not hasattr(auction, 'sold_players'):
                auction.sold_players = {}
            auction.sold_players[auction.current_player.user_id] = bot_instance._record_auction_result(
                auction.current_player, 'UNSOLD', 0, None
            )
...
async def unsold_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Initialize unsold_players if it doesn't exist (for old auctions)
        if not hasattr(auction, 'unsold_players'):
            auction.unsold_players = {}
        ...
        for i, player_data in enumerate(auction.unsold_players.values(), 1):
            # Handle both dict format and legacy direct object format
            player = player_data['player'] if isinstance(player_data, dict) else player_data
...
async def addpt_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        try:
            ...
            # Everything between auction_id and amount is player name and team name
            # We need at least 2 more args: player_name and team_name
            ...
            # Last middle arg is team name, everything else is player name
...
async def participants_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Only host/admin can view participants
        if auction.creator_id != user.id and not bot_instance.is_admin(user.id):
...
async def delete_auction_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Check if auction exists in proposals or approved auctions
        auction_found = False
        auction_name = ""
        
        # Check approved auctions
...
async def addpauc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Extract player names from the replied message text
        ...
        # Remove @usernames from text to get plain names
        ...
        # Clean up plain names - remove extra spaces and empty strings
        plain_names = [name.strip() for name in plain_names if name.strip() and len(name.strip()) > 2]
        
        # Combine both lists
        all_players = []
        
        # Add @usernames with @ prefix
        ...
        # Add each player name directly to auction
        for player_name in all_players:
            try:
                # Generate a unique player ID based on player name
                ...
                # Add directly to approved players - simple!
                ...
                # Add to player_queue if auction is active (for immediate bidding)
                if auction.status == "active":
...
async def removepauc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Extract usernames from the replied message
        replied_text = update.message.reply_to_message.text or update.message.reply_to_message.caption or ""
        
        # Find all usernames in the message (with or without @)
        import re
...
# ====================================
# AUCTION COUNTDOWN AND AUTOMATION
# ====================================
...
def next_player(auction: ApprovedAuction) -> str:
    ...
    if auction.current_player_index < len(player_ids):
        # Award current player to highest bidder
...
# Timer function removed - manual control only
...
def parse_bid_amount(bid_text):
    ...
    # Remove 'cr' or 'Cr' if present (case insensitive)
    if bid_text.lower().endswith('cr'):
        bid_text = bid_text[:-2].strip()
    
    # Handle empty after removing 'cr'
    if not bid_text:
        return None
    
    try:
        # Convert to float first, then return as is (crores)
        amount = float(bid_text)
        # Validate reasonable range (0.1 to 1000 crores)
        if 0.1 <= amount <= 1000:
            return amount
        else:
            return None
...
def register_commands(application):
    ...
    # OLD BID SYSTEM REMOVED - NOW USING DIRECT CHAT BIDDING
    ...
    # slb command removed - use shardlb instead
